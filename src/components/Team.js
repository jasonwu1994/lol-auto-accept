...
const Team = ({title, team}) => {
  return (
    <>
      <h2>{title}{getMaxCount(team)}</h2>
      {team.map((group, groupIndex) => (
        <React.Fragment key={`group-${groupIndex}`}>
          {group.map((player, playerIndex) => {
            let str = `${apiUtils.getChampionNameById(player.championId)} ${player.summonerInternalName} ${player.selectedPosition ?? ''}`;
            return (
...
function getMaxCount(inputArray) {
